...
from muotti._decorators import MAPPING_PROFILE_CACHE_KEY, MAPPING_PROTOCOL_CACHE_KEY
from muotti._profiles import Profile
from muotti._protocol import ObjectProtocol, ProtocolRegistry
from muotti._runner import MappingRunner
from muotti._sequence import MappingSequence
from muotti._spec import NO_OVERRIDE
from muotti._value import BasicValueConverter, ValueConverter
from muotti.errors import MappingConfigurationError, MappingError, ValidationError


class Mapper:
    def __init__(self, converter: ValueConverter | None = None) -> None:
        self.registry = ProtocolRegistry()
        self.converter = converter if converter is not None else BasicValueConverter()
        self._sequences: dict[int, MappingSequence[Any, Any]] = {}

    def load_from_cache(self, cache: Cache) -> None:
