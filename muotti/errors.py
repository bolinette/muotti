...
class MuottiError(Exception):
    def __init__(self, message: str) -> None:
        super().__init__(message)
        self.message = message


class MappingError(MuottiError):
    def __init__(
        self,
        message: str,
        *,
        ...
        MuottiError.__init__(self, ", ".join(parts))
        self.dest = dest
        self.src = src
...
class MappingConfigurationError(MuottiError):
    def __init__(self, problems: list[str]) -> None:
        MuottiError.__init__(self, f"{len(problems)} invalid mapping(s):\n" + "\n".join(f"  - {p}" for p in problems))
